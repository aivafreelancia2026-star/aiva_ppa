...
const CATEGORY_COLORS: Record<string, string> = {
  ...
  Beverages: '#f59e0b', Snacks: '#ec4899', Household: '#64748b', 'Personal Care': '#fbbf24', Other: '#94a3b8',
}
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-theme-primary">Shopping List</h1>
      ...
      {items.length > 0 && (
        <div className="glass-card p-4">
          ...
          <div className="h-2 rounded-full overflow-hidden" style={{ background: 'rgba(245,158,11,0.12)' }}>
            <motion.div
        ...
        <div className="flex rounded-xl overflow-hidden border border-amber-500/20">
          {(['all', 'pending', 'purchased'] as const).map(f => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={cn('px-4 py-2 text-xs font-medium capitalize transition-colors', filter === f ? 'bg-amber-600/20 text-amber-600 dark:text-amber-300' : 'text-theme-secondary hover:text-theme-primary')}
            >
              {f}
            </button>
          ))}
      ...
      {loading ? (
        ...
        <div className="text-center py-16">
          <ShoppingBag className="w-12 h-12 text-theme-secondary mx-auto mb-3" />
          <h3 className="text-theme-secondary font-medium">Your shopping list is empty</h3>
          ...
          {Object.entries(grouped).map(([category, catItems]) => (
            <div key={category}>
              <div className="flex items-center gap-2 mb-2">
                <div className="w-2 h-2 rounded-full" style={{ background: CATEGORY_COLORS[category] ?? '#f59e0b' }} />
                  ...
                  {catItems.map(item => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <span className={cn('text-sm font-medium text-theme-primary', item.is_purchased && 'line-through text-theme-muted')}>
                          {item.name}
                        </span>
                      ...
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={() => { setEditItem(item); setShowModal(true) }} className="p-1.5 rounded-lg hover:bg-theme-elevated text-theme-secondary hover:text-theme-primary transition-all">
                          <Edit2 className="w-3.5 h-3.5" />
                        </button>
                        <button onClick={() => deleteItem(item.id)} className="p-1.5 rounded-lg hover:bg-red-500/10 text-theme-secondary hover:text-red-400 transition-all">
    ...
    >
      ...
      >
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-lg font-semibold text-theme-primary">{item ? 'Edit Item' : 'Add Item'}</h2>
